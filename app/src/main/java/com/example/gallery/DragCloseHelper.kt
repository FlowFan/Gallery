...
class DragCloseHelper(
     ...
     * 获取状态栏的高度
     */
    private fun getStatusBarHeight(context: AppCompatActivity) =
        ViewCompat.getRootWindowInsets(context.window.decorView)?.displayCutout?.safeInsetTop ?: 0

    /**
     * 获取导航栏的高度
     */
    private fun getNavigationBarHeight(context: AppCompatActivity) =
        ViewCompat.getRootWindowInsets(context.window.decorView)?.displayCutout?.safeInsetBottom ?: 0
